...
const handleAutoLogout = async () => {
  ...
  const checkAuth = async () => {
    try {
      ...
      // Clear any stored auth data using localStorage (not Appwrite storage)
...
const logout = async () => {
  ...
  try {
    ...
    // Use localStorage, not storage.removeItem()
    localStorage.removeItem('authenticated');
    localStorage.removeItem('email');
    console.log('Successfully logged out');
    navigate('/', { replace: true });
  ...
  // Add this condition to prevent showing content when not authenticated
  if (!isAuthenticated) {
    // Don't show anything - the redirect in checkAuth() will handle it
    return null;
  }
